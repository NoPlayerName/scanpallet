...
<div class="container">
	...
	<label style="font-size:20pt">Form Mutasi Pallet</label>
	</legend>
</div>
<form id='myform'>
<div class="container">
	<div class="form-group">
		...
		<input type="text" name="nopall" id="nopall" style="font-size:15pt;height:35px;width:100%" tabIndex="3" value="" autocomplete="off" onkeypress="if(event.keyCode==13) document.getElementById('button1').click()">
		</p>
		<label>No Rak :</label>
		<input type="text" name="norak" id="norak" style="font-size:15pt;height:35px;width:100%" tabIndex="3" value="" autocomplete="off" onkeypress="if(event.keyCode==13) document.getElementById('button2').click()">
	</div>	
 </div>
 </fieldset>
 <input type="text" name="pco" id="pco" style="font-size:15pt;height:35px;width:100%" tabIndex="3" value="" autocomplete="off" hidden>
...
<div class="modal fade container vertical-alignment-helper" id="modalitem" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	...
	<br><br><br><br><br>
		<div class="modal-content">
			...
			<div class="modal-body" id="dataitem" style='font-size:30px' align='center' hidden>
			</div>
			<div class="modal-body" id="datarak" style='font-size:20px;color:red' align='center' hidden>
			</div>
			<div class="modal-body" id="datalorong" style='font-size:25px;color:green' align='center' hidden>
				<h4>Tidak ada referensi No. Rak, silahkan scan No. Lorong !</h4>
			</div>
...
$(document).on('click', '.button1', function () {
	var nopallco = document.getElementById('nopall').value;
			
		$.ajax({
			url: 'ajax/rakcross.php',	
			method: 'post',		
			data: {nopallco:nopallco},		
			success:function(data){		
			//$('#h2').html(data);
			document.getElementById('refrak').value = data;			
			$('#divmodal').modal("show");					
...
$(document).on('click', '.button2', function () {
	var nopall = document.getElementById('nopall').value;
	var norak = document.getElementById('norak').value;
	
	if (nopall == ""){
		$('#h2').html('No Pallet Tidak Boleh Kosong');
		$('#nopall').focus();
		document.getElementById('norak').value = ''
	}else if (norak == ""){
		$('#h2').html('No Rak Tidak Boleh Kosong');
		$('#norak').focus();
	}else{
	
		var lrmp = 'LR';
		var war = 'No Rak harus sesuai dengan referensi No Rak !';
		var warlr = 'No Rak harus scan No Lorong !';
		 
		var pall = document.getElementById('nopall').value;
		var pallmp = document.getElementById('nopall').value;
...
/* $(document).on('click', '.button2', function () {
	...
	var lorong = "Tidak ada referensi no. rak, silahkan scan no lorong !";
		...
		$.ajax({
			...
			if (refrak = 'LR'){
				$('#dataitem').hide();
				$('#datalorong').show();
				document.getElementById('refrak').value = 'LR';
			}else{
				$('#datalorong').hide();
				$('#dataitem').show();
				$('#dataitem').html(data);
...
$('#modalitem').on('hidden.bs.modal', function (e) {
     var nopall = document.getElementById('nopall').value; 
     var refrak = document.getElementById('refrak').value; 
	 var rak = refrak.substring(0, 2)
	 var n = refrak.length;
	 
	 if (rak == 'LR'){
		$("#nopall").focus();
		document.getElementById('nopall').value = '';
		document.getElementById('refrak').value = '';
	 }else if (n == 4){
		$("#norak").focus();
	 }else{
		$("#nopall").focus();
 ...
 $(document).on('click', '.button3', function () {
	...
	var partlr = document.getElementById('partno').value;
	...
	if (refrak == 'LR'){
		...
		if (res != refrak){
			...
			url: 'ajax/view.php',	
			method: 'post',		
			data: {lr:lr,partlr:partlr,pall:pall,rak:rak,tbl:tbl},		
			success:function(data){		
			$('#h2').html(data);
			document.getElementById("myform").reset();
			$("#partno").focus();
